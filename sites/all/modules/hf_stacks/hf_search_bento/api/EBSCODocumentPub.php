...
require_once 'EBSCOAPIPub.php';
require_once 'EBSCORecordPub.php';


class EBSCODocumentPub
{
    /**
...
//    private $limit = 10;
    ...
    {
        $this->eds = new EBSCOAPIPub(array(
            'password'     => variable_get('stacks_eds_pass'),
            'user'         => variable_get('stacks_eds_user'),
            'profile'      => variable_get('stacks_eds_profile'),
            'interface'    => variable_get('stacks_eds_org'),
            'organization' => variable_get('stacks_eds_org'),
            'guest'        => 'y',
            //'log'          => variable_get('ebsco_log')
            'json'         => (variable_get('stacks_search_bento_format') == 'json') ? true : false,
        ));

        $this->params = $params ? $params : $_REQUEST;
//        $this->limit = variable_get('stacks_search_bento_limit_eds') ? variable_get('stacks_search_bento_limit_eds') : $this->limit;
    }
    ...
    public function search()
    {
        ...
        $includeFacet = isset($this->params['includeFacet']) ? $this->params['includeFacet'] : 'y';
        $action = isset($this->params['action']) ? $this->params['action'] : null;

        $this->results = $this->eds->apiSearch($search, $filter, $page, $limit, $sort, $amount, $mode, $includeFacet, $action);
    ...
    public function record()
    {
        if (empty($this->record) && !(empty($this->result))) {
            $this->record = new EBSCORecordPub($this->result);
        }
    ...
    public function records()
    {
        if (empty($this->records) && !(empty($this->results))) {
            foreach($this->results['documents'] as $result) {
                $this->records[] = new EBSCORecordPub($result);
            }
        }
        ...
        if ($lastSearch) {
            ...
            if ($id) {
                ...
                if (!isset($lastSearch['records'][$index + 1])) {
                    $params['page'] += 1;
                    $driver = new EBSCODocumentPub($params);
                    $driver->search();
                    $query = drupal_http_build_query($params);
