...
require_once 'EBSCOConnectorPub.php';
require_once 'EBSCOResponsePub.php';
...
class EBSCOAPIPub
{
    ...
    /**
     * The expiry timestamp for this the Authentication Token
     * @global string
     */
    private $authenticationTokenExpiry;
    ...
    /**
     * The expiry timestamp for this the Session Token
     * @global string
     */
    ...
    {
        $this->config = $config;
        $this->authenticationTokenExpiry = variable_get('hf_stacks_search_eds_authenticationToken_expiry');
        $this->sessionTokenExpiry = variable_get('hf_stacks_search_eds_sessionToken_expiry');
    ...
    /**
     * Getter for authentication token expiry timestamp
     * @return timestamp
     * @access public
     */
    public function getAuthenticationTokenExpiry()
    {
        return $this->authenticationTokenExpiry;
    }
    ...
    /**
     * Getter for session token expiry timestamp
     * @return timestamp
     * @access public
     */
    ...
    public function connector()
    {
        if (empty($this->connector)) {
            $this->connector = new EBSCOConnectorPub($this->config);
        }
    ...
    {
        $responseObj = new EBSCOResponsePub($response);
        return $responseObj;
    }
    ...
    protected function request($action, $params = null, $attempts = 5)
    {
        ...
        $authenticationTokenExpiry = $this->getAuthenticationTokenExpiry();
        $sessionTokenExpiry = $this->getSessionTokenExpiry();
        ...
        if (empty($authenticationToken) || empty($sessionToken) || empty($authenticationTokenExpiry) || empty($sessionTokenExpiry)) {
        ...
        //If there is an expiry timestamp, check to see if tokens are not expired, else renew all tokens
...
//        }
        ...
        if ($this->isError($response)) {
            ...
            switch ($code) {
                ...
                case EBSCOConnectorPub::EDS_AUTH_TOKEN_INVALID:
                    ...
                    break;
                case EBSCOConnectorPub::EDS_SESSION_TOKEN_INVALID:
                    $result = $this->apiAuthenticationAndSessionToken();
                    if ($this->isError($result)) {
                        // Any error should terminate the request immediately
                        // in order to prevent infinite recursion
        ...
        //$info = $this->connector()->requestInfo($params, $headers);
     ...
     * @param string $includeFacet The toggle for including facets in results
     *
    ...
    public function apiSearch($search, $filters,
        $start = 1, $limit = 10, $sortBy = 'relevance', $amount = 'detailed', $mode = 'all', $includeFacet = 'y', $action = null, $publication
    ) {
        $query = array();
        ...
        $params = array(
            ...
            'includefacets'  => $includeFacet,
            'resultsperpage' => $limit,
            'pagenumber'     => $start,
            // Specifies whether or not to include highlighting in the search results
            'highlight'      => 'y',
            'action'         => ($action != null) ? 'addfacetfilter(SourceType:' . $action . ')' : '',
        );
    ...
    private function handleError($error) {
        ...
        switch($errorCode) {
            ...
            case EBSCOConnectorPub::EDS_INVALID_ARGUMENT_VALUE:
    ...
    protected function writeSession($key, $value)
    {
        if(!empty($key) && !empty($value)) {
            if($key == 'authenticationToken' || $key == 'sessionToken') {
                variable_set('hf_stacks_search_eds_' . $key, $value);
                //Set time interval to preconfigured time
                variable_set('hf_stacks_search_eds_' . $key . '_expiry', date("d-m-Y h:i:s",time() + (5 * 60)));
            } else {
    ...
    {
        if($key == 'authenticationToken' || $key == 'sessionToken') {
            $value = variable_get('hf_stacks_search_eds_' . $key);
        }else {
