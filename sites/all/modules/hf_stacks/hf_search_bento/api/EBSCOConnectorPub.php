...
/**
    ...
    private static $pub_point = 'http://eds-api.ebscohost.com/EDSAPI/publication';
...
//        }
    ...
    public function requestAuthenticationToken()
    {
        ...
        $json = $this->json;
        if($json) {
            $data = array("UserId" => $this->userId, "Password" => $this->password, "InterfaceId" => $this->interfaceId);
...
BODY;
        ...
        $response = $this->request($url, $params, array(), 'POST', $json);
        return $response;
    }
     ...
     * @access public
     */
    public function requestSearch($params, $headers, $json)
    {
      $url = self::$pub_point . '/Search';

      $response = $this->request($url, $params, $headers, 'GET', $json);
      return $response;
    }
     ...
     * @param bool   $json        Return Json or XML
     *
     * @return object             SimpleXml or PEAR_Error
     * @access protected
     */
            ...
            switch ($code) {
                case self::HTTP_OK:
                    if($json) {
                        ...
                        try {
                            ...
                            $return = new EBSCOException($json);
                        }
                    } else {
                ...
                case self::HTTP_BAD_REQUEST:
                    if($json) {
                        ...
                        $isError = isset($json->ErrorNumber) || isset($json->ErrorCode);
                        if($isError) {
                            $error = ''; $code = 0;
                            if (isset($json->DetailedErrorDescription) && !empty($json->DetailedErrorDescription)) {
                                $error = (string) $json->DetailedErrorDescription;
                            } else if (isset($json->ErrorDescription)) {
                                $error = (string) $json->ErrorDescription;
                            } else if (isset($json->Reason)) {
                                $error = (string) $json->Reason;
                            }
                            if (isset($json->ErrorNumber)) {
                                $code = (integer) $json->ErrorNumber;
                            } else if (isset($json->ErrorCode)) {
                                $code = (integer) $json->ErrorCode;
                            }
...
//        if ($this->logAPIRequests) {
            ...
            $message = $this->toString($response);
            //$this->logger->log($message, Zend_Log::ERR);

            //error_log($message);
            //error_log($xml_str);
            //error_log("Elapse Time from ". $timestart . " to " . $timeend . " = " . $timeend - $timestart);
//        }
